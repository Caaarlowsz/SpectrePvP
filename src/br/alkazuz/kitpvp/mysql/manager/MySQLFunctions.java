package br.alkazuz.kitpvp.mysql.manager;
...
public class MySQLFunctions {
	...
	public static void setKills(Player p, int i) {
		try {
			Data.statement.executeUpdate(
					"UPDATE kitpvp SET Kills='" + i + "' WHERE UUID='" + p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
	...
	public static void setMoney(OfflinePlayer p, int i) {
		try {
			Data.statement.executeUpdate(
					"UPDATE kitpvp SET Money='" + i + "' WHERE UUID='" + p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
	...
	public static void setWins(OfflinePlayer p, int i) {
		try {
			Data.statement.executeUpdate(
					"UPDATE kitpvp SET Vitoria='" + i + "' WHERE UUID='" + p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
	...
	public static void addWins(Player p, int i) {
		try {
			Data.statement.executeUpdate("UPDATE kitpvp SET Vitoria='" + (getWins(p) + i) + "' WHERE UUID='"
	...
	public static void addLoser(Player p, int i) {
		try {
			Data.statement.executeUpdate("UPDATE kitpvp SET Derrotas='" + (getLoser(p) + i) + "' WHERE UUID='"
					+ p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
	...
	public static int getLoser(Player p) {
		try {
			ResultSet res = Data.statement
					.executeQuery("SELECT * FROM kitpvp WHERE UUID='" + p.getUniqueId().toString() + "'");
			res.next();
	...
	public static void setDeath(OfflinePlayer p, int i) {
		try {
			Data.statement.executeUpdate(
					"UPDATE kitpvp SET Deaths='" + i + "' WHERE UUID='" + p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
	...
	public static void loadPlayer(Player p) {
		...
		LogManager.createLogStatus("Carregado o Status de " + p.getName() + " Kills:" + Kills(p) + ",Money:" + Money(p)
				+ ",Mortes:" + Deaths(p) + ",Caixas:" + Caixas(p));
	}

	public static void savePlayer(Player p) {
		try {
			Data.statement.executeUpdate("UPDATE kitpvp SET Kills='" + Status.getkills(p) + "', Money='"
					+ Status.getCoins(p) + "', Caixas='" + Status.getCaixas(p) + "', Deaths='" + Status.getDeaths(p)
	...
	public static void CriarTabela() {
		try {
			PreparedStatement ps = kMySQL.getStatement(
					"CREATE TABLE IF NOT EXISTS kitpvp (UUID VARCHAR(100), NICK VARCHAR(100), Money INT(100), Kills INT(100), Deaths INT(100), Vitoria INT(100), Derrotas INT(100), Caixas INT(100))");
			ps.executeUpdate();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public static void Registro(UUID p, String nick) {
		try {
			Data.statement.executeUpdate(
					"INSERT INTO kitpvp (UUID,NICK, Money, Kills, Deaths, Vitoria, Derrotas, Caixas) VALUES ('"
							+ p.toString() + "','" + nick + "', '0', '0', '0', '0', '0', '0')");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public static void Registro(Player p) {
		try {
			Data.statement.executeUpdate(
					"INSERT INTO kitpvp (UUID,NICK, Money, Kills, Deaths, Vitoria, Derrotas, Caixas) VALUES ('"
							+ p.getUniqueId().toString() + "','" + p.getName() + "', '0', '0', '0', '0', '0', '0')");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public static List<String> getPlayersall() {
		...
		try {
			...
			int index = 1;
			while (rs.next()) {
				if (list2.contains(rs.getString("NICK")))
					continue;
				if (list.size() >= 10)
					break;
	...
	public static void addKills(Player p, int i) {
		try {
			Data.statement.executeUpdate("UPDATE kitpvp SET Kills='" + (Deaths(p) + i) + "' WHERE UUID='"
					+ p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
	...
	public static void addMoney(Player p, int i) {
		try {
			Data.statement.executeUpdate(
					"UPDATE kitpvp SET Money='" + (Money(p) + i) + "' WHERE UUID='" + p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
	...
	public static void addDeaths(Player p, int i) {
		try {
			Data.statement.executeUpdate("UPDATE kitpvp SET Deaths='" + (Deaths(p) + i) + "' WHERE UUID='"
					+ p.getUniqueId().toString() + "'");
		} catch (Exception e) {
		}
	}
