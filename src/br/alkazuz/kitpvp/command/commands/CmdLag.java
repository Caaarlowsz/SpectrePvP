...
import br.alkazuz.kitpvp.api.Lag;
...
public class CmdLag implements CommandExecutor {
	...
	public boolean onCommand(CommandSender arg0, Command arg1, String arg2, String[] arg3) {
		...
		if (arg1.getName().equalsIgnoreCase("ping")) {
			...
			p.sendMessage("§e§lPING §6" + target.getName() + " §festá com um ping de §a"
		...
		if (arg1.getName().equalsIgnoreCase("lag")) {
			...
			p.sendMessage("§7Você está no Servidor§8: §2§lSpectrePvP§8, §7online aqui§8: §a"
					+ Bukkit.getOnlinePlayers().length);
			p.sendMessage("  §c- §7Uso de processamento§8: §a" + usedCPU + " %§8, §7lag§8: §a"
					+ (usedCPU >= 90 ? "sim" : "nao"));
			p.sendMessage("  §c- §7Sobrecarregamento geral§8: §a" + RAM_USED_PERCENTAGE + " %§8, §7lag§8: §a"
					+ (RAM_USED_PERCENTAGE >= 90 ? "sim" : "nao"));
			...
			p.sendMessage(getString((RAM_USED_PERCENTAGE >= 90 ? "sim" : "nao"), (ping >= 450 ? "sim" : "nao"),
					(usedCPU >= 90 ? "sim" : "nao")));
			p.sendMessage("§7 ");
			return true;
		}
