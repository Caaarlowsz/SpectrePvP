	...
	public boolean onCommand(CommandSender Sender, Command Cmd, String Label, String[] Args) {
		...
		if (Cmd.getName().equalsIgnoreCase("tp")) {
			...
			if (Args.length > 2 || Args.length == 3) {
				...
				p.sendMessage(
						MessageAPI.Command_Succes + "Você foi teleportado para §2(§a" + X + " " + Y + " " + Z + "§2)");
				return true;
			}
			...
			if (Args.length > 1) {
				...
				p.sendMessage(MessageAPI.Command_Succes + "Você teleportou o jogador(a) §2(§a" + t.getName()
						+ "§2) §aaté você");
				return true;
			}
			p.teleport(t.getLocation());
			p.sendMessage(
					MessageAPI.Command_Succes + "Você foi teleportado até o jogador(a) §2(§a" + t.getName() + "§2)");
