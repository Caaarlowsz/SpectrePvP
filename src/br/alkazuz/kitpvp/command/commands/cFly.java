...
public class cFly implements CommandExecutor {
	...
	public boolean onCommand(CommandSender Sender, Command Cmd, String Label, String[] Args) {
		...
		if (Cmd.getName().equalsIgnoreCase("fly")) {
			...
			if (Args.length > 0) {
				if ((!GroupAPI.getGroup(p).equalsIgnoreCase("Dono")
						&& (!GroupAPI.getGroup(p).equalsIgnoreCase("Gerente")
								&& (!GroupAPI.getGroup(p).equalsIgnoreCase("Administrador"))))) {
					p.sendMessage(MessageAPI.Command_Error + "Comando inexistente ou você não tem acesso");
					return true;
				}
				...
				if (!Fly.contains(target)) {
					target.sendMessage(
							MessageAPI.Command_Succes + "Seu modo fly foi §2ativado §apelo o player §e" + p.getName());
					p.sendMessage(
							MessageAPI.Command_Succes + "Você §2ativou §ao modo fly foi de §e" + target.getName());
					Fly.add(target);
					target.setAllowFlight(true);
				} else {
					target.sendMessage(MessageAPI.Command_Succes + "Seu modo fly foi §4desativado §apelo o player §4"
							+ p.getName());
					p.sendMessage(
							MessageAPI.Command_Succes + "Você §4desativou §ao modo fly foi de §4" + target.getName());
					Fly.remove(target);
