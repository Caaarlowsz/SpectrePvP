...
import br.alkazuz.kitpvp.clan.Clan;
import br.alkazuz.kitpvp.clan.ClanManager;
			...
			try {
				...
				if (Args[0].equalsIgnoreCase("convidar")) {
					...
					if (!list.contains(nick)) {
						...
						if (vic != null) {
							vic.sendMessage(" ");
							vic.sendMessage("§6§lClan §fVocê recebeu um Convite do clan §f"
									+ ClanAPI.getClanPlayerString(p) + " §f(" + ClanConfig.getConfig()
											.getString(ClanAPI.getClanPlayerString(p) + ".Tag").replace("&", "§")
									+ "§f)");
							vic.sendMessage("§6§lClan §fUse §a/clan aceitar " + ClanAPI.getClanPlayerString(p)
						...
						ClanAPI.senMessage(ClanAPI.getClanPlayerString(p),
								"§a# §f" + nick + " §afoi convidado para o Clan");
						return true;
					} else {
				...
				if (Args[0].equalsIgnoreCase("info")) {
					...
					p.sendMessage("§fTag do Clan: " + ClanConfig.getConfig()
							.getString(ClanAPI.getClanPlayerString(p) + ".Tag").replace("&", "§"));
				...
				if (Args[0].equalsIgnoreCase("expulsar")) {
					...
					if (vic == null) {
						...
						ClanAPI.senMessage(ClanAPI.getClanPlayerString(p),
								"§c# §f" + p.getName() + " §fexpulsou o Jogador §e" + vicoff.getName());
						return true;
					}
					...
					ClanAPI.senMessage(ClanAPI.getClanPlayerString(p),
							"§c# §f" + p.getName() + " §fexpulsou o Jogador §e" + vic.getName());
				...
				if (Args[0].equalsIgnoreCase("deletar")) {
					...
					Bukkit.broadcastMessage(
							"§6§lCLAN §e" + p.getName() + " §fdeletou o Clan §c" + ClanAPI.getClanPlayerString(p));
