...
public class cTag implements CommandExecutor {
	...
	public boolean onCommand(CommandSender Sender, Command Cmd, String Label, String[] Args) {
		...
		if (Cmd.getName().equalsIgnoreCase("tag")) {
			...
			if (!GroupAPI.getGroup(p).equalsIgnoreCase(tag) && !GroupAPI.getGroup(p).equalsIgnoreCase("Dono")
					&& !tag.equalsIgnoreCase("Membro")) {
		...
		if (Cmd.getName().equalsIgnoreCase("tags")) {
			...
			p.sendMessage(MessageAPI.Command_Succes + "Suas Tags: " + grupos
					+ (GroupAPI.getGroup(p).equalsIgnoreCase("Membro") ? "" : ", §fMembro"));
		}
		return false;
	}
