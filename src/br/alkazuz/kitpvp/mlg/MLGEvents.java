...
public class MLGEvents implements Listener {
			...
			if (WarpAPI.getWarp(p).equalsIgnoreCase("MLG")) {
				if (e.getCause() == EntityDamageEvent.DamageCause.FALL) {
					if (acertou.contains(p))
						return;
					p.sendMessage(MessageAPI.Command_Error + "Voc� errou o MLG");
					MLGAPI.addErros(p);
					MLGAPI.clearKs(p);
	...
	public void a(final PlayerInteractEvent e) {
		...
		if (WarpAPI.getWarp(p).equalsIgnoreCase("MLG") && e.getAction() == Action.RIGHT_CLICK_BLOCK
				&& e.getClickedBlock().getLocation().getBlockY() > p.getLocation().getBlockY() + 2) {
			e.setCancelled(true);
			p.updateInventory();
		}
	}
	...
	public void wefwef(PlayerBucketEmptyEvent e) {
		...
		if (!WarpAPI.getWarp(e.getPlayer()).equalsIgnoreCase("MLG"))
			return;
		double y = WarpConfig.getConfig().getDouble("Warps.MLG.Y");
		e.setCancelled(true);
		p.updateInventory();
		if (e.getBucket() == Material.WATER_BUCKET && e.getBlockFace() == BlockFace.UP
			...
			if (p.getLocation().subtract(0.0, 3.0, 0.0).getBlock().getType() != Material.AIR) {
				e.getPlayer().sendMessage(MessageAPI.Command_Succes + "Voc� acertou o MLG");
