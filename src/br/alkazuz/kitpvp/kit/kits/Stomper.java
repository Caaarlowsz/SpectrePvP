...
public class Stomper extends Kit implements Listener {
	...
	public Stomper() {
		/*
		 * super("Stomper", new
		 * String[]{" ","§7Pisoteie um Jogador","§7usando sua bota poderosa"}, new
		 * ItemStack(Material.IRON_BOOTS), "kit.stomper",KitType.AVANCED);
		 */
		super("Stomper", 5000, false, new ItemStack(Material.IRON_BOOTS), false, "kit.stomper", false,
				new String[] { " ", " §e§l- §7Pisoteie um Jogador", " §e§l- §7usando sua bota poderosa" });
	}
		...
		if (e.getCause() == EntityDamageEvent.DamageCause.FALL) {
			if (KitAPI.getKitName(p).equalsIgnoreCase("Stomper")) {
				for (Entity ent : p.getNearbyEntities(5.0D, 3.0D, 5.0D)) {
					...
					if (plr.isSneaking() || KitAPI.getKitName(plr).equalsIgnoreCase("antistomper"))
						continue;
